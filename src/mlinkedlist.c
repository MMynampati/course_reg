...
void MDeleteList(mlist_t* list) {
    ...
    while (list->head != NULL){
        void* retval = MRemoveFromHead(list);
        list->deleter(retval);
    }
    list->length = 0;
}
 ...
 void MSortList(mlist_t* list) {
    ...
    mlist_t* new_list = malloc(sizeof(mlist_t));	
    ...
    for (; i < len; ++i)
    {
        void* val = MRemoveFromTail(list);
        MInsertInOrder(new_list, val); 
    }
